...
{
    P1DIR &= 0b11111101; // sets port1 but 1 as a input
    P1REN |= 0b00000010; // Enables pullup Resistor
    P1OUT |= 0b00000010; //Makes Port1 pins 1 to output Pullup
...
{
   P5DIR  |= 0b10000000; //sets to output
   P5SEL0 |= 0b10000000; // Sets sel0 port 5 bit7 to 1 to enable Timer2
   P5SEL1 &= 0b01111111; // Sets Sel1 Port 5 bit7 to 0 to enable Timer2
  // Use the port direction register to configure pins as inputs and output
  // 	-A 1 sets a pin as an output. Use |= with a bit mask to set 1's.
  // 	-A 0 sets a pin as an input. Use &= with a bit mask to set 0's.
  // Use the resistor enable register to enable resistors for inputs that need them
...
void Port7_Init(void){
    P7DIR  |= 0b00100000; //sets to output
    P7SEL0 |= 0b00100000; // Sets sel0 port 7 bit5 to 1 to enable Timer1
    P7SEL1 &= 0b11011111; // Sets Sel1 Port 7 bit5 to 0 to enable Timer1
  // Use the port direction register to configure pins as inputs and output
  // 	-A 1 sets a pin as an output. Use |= with a bit mask to set 1's.
  // 	-A 0 sets a pin as an input. Use &= with a bit mask to set 0's.
  // Use the resistor enable register to enable resistors for inputs that need them
