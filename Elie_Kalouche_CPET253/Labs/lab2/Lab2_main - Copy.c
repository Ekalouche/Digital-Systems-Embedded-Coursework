...
//The LED is controlled by bits 0, 1 and 2 on PORT2
 P2OUT &= ~0b00000111;   //first turn off all colors
 P2OUT |= (color & 0b00000111);    //second turn on the input color
}
...
void main(void)
{
       ...
       enum color_blinker {Light_Green,Light_Red,Light_Blue,Light_Purple} state, prevState;

       state - Light_Green;                     //start state
       prevState - !Light_Green;                 //used to know when the state has changed
       ...
       while(1)
       {
          ...
          case Light_Green:
              ...
              if(isNewState)
                 ...
                 LED_Color(GREEN);   //state business - turn light green
                 stateTimer++;
              if(stateTimer >= 100)
              {
                 state = Light_Red;
              ...
              if(isNewState)
                            {
                              ...
                              LED_Color(RED);
                           //state business - turn light green
                              stateTimer++;
                           if(stateTimer >= 100)
                           {
                              state = Light_Blue;
